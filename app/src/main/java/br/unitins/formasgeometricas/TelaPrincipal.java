    ...
    public boolean onTouchEvent(MotionEvent event){
        ...
        switch (event.getAction()){
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                if(tocar(x,yCalc,render.botaoP)
                        ){
                    abrirAlert(new Paralelogramo(render.gl, TAMANHO), 0);
                }

                if(tocar(x,yCalc,render.botaoQ)
                        ){
                    abrirAlert(new Quadrado(render.gl, TAMANHO),0);
                }

                if(tocar(x,yCalc,render.botaoT)
                        ){
                   abrirAlert(new Triangulo(render.gl, TAMANHO),0);
                }

                for (Geometria forma: render.formas){
...
//                    abrirAlert(g,3);
    ...
    //retorna a altura do dispositivo, ao inves da altura do layout
    private float getAlturaReal(){
        //DisplayMetrics é responsavel pelas informações de display, como
        //tamanho, densidade e escala de fonte
    ...
    public void abrirAlert(final Geometria g, int i){

        final EditText angulo = new EditText(this);
        angulo.setHint("0");
...
class Render implements GLSurfaceView.Renderer{
    ...
    public void onSurfaceChanged(GL10 gl, int width, int height){
        ...
        botaoP = new Paralelogramo(gl, TelaPrincipal.TAMANHO);
        botaoQ = new Quadrado(gl, TelaPrincipal.TAMANHO);
        botaoT = new Triangulo(gl, TelaPrincipal.TAMANHO);
